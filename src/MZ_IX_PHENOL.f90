...
!----------------------------------------------------------------------
	...
	REAL            GDDAE
      ...
      REAL            PLTPOP         
      REAL            PSKER          
      REAL            PEAR  ! JIL Modifies ear C partitioning
C      REAL            PRLF  ! JIL Prolificacy level
      REAL            PSTM  ! JIL Modifies stem C partitioning
      REAL            RATEIN         
      REAL            ROPT           
      REAL            ROWSPC         
      REAL            RUE
...
!            READ(LUNIO,60,IOSTAT=ERR) PLTPOP,SDEPTH
            READ(LUNIO,60,IOSTAT=ERR) YREMRG,PLTPOP,ROWSPC,SDEPTH
            LNUM = LNUM + 1
! 60         FORMAT(25X,F5.2,25X,F5.2)
 60         FORMAT(11X,I7,7X,F5.2,13X,F5.2,7X,F5.2)
            IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILEIO,LNUM)
          ENDIF
!     -----------------------------------------------------------------
...
!     %                   P1,P2,P5,G2,G3,PHINT ; LNUM = LNUM + 1 
     %                   P1,P2,P5,G2,G3,PHINT,AX,LX ; LNUM = LNUM + 1 
            IF (ERR .NE. 0) CALL ERROR(ERRKEY,ERR,FILEIO,LNUM)
...
!-----------------------------------------------------------------------
     ...
     &          C255(1:1) .NE. '*') THEN
!       JIL Read new parameters PSTM, PEAR     
...
!              IF (SUMDTT .LT. P9) RETURN 
	          ...
	          IF (SUMDTT .LT. P9) RETURN
	        ELSE
!	          IF (SUMDTT .LT. P9 .AND. YRDOY .LT. YREMRG) RETURN
	        ...
	        ENDIF
              ...
              ISTAGE = 1
              SUMDTT = SUMDTT - P9
! JIL 09/10/2007 Initialization causing problems with new leaf area
!              TLNO   = 30.0
              TLNO   = 0.0
! JIL 09/10/2007Thermal time after emergence
	        GDDAE = SUMDTT
...
! JIL 09/10/2007 Continuous phenological scale (0=Emergence; 1=Flowering; 2= Physiol Mat)
              Z2STAGE = 0.25 * SUMDTT/P1
		      Z2STAGE = AMIN1(0.25, Z2STAGE)
...
!     try using 25* instead of 20*
              ...
              !RATEIN - floral induction rate driven by daylength
              ! and photoperiod sensitivity for maize (different 
              ! for SG, ML
...
! JIL 09/10/2007 Continuous phenological scale (0=Emergence; 1=Flowering; 2= Physiol Mat)
              Z2STAGE = 0.25 + SIND*0.1
		      Z2STAGE = AMIN1(0.35, Z2STAGE)
              ...
              STGDOY(ISTAGE) = YRDOY          
              ISTAGE = 3
! JIL 09/10/2007 We don't need this. This is for other cereals
!              XNTI   = SUMDTT/43.0  
           !Next 2 lines: Change implemented at CIMMYT 1999 - JTR,US
              TLNO    = SUMDTT/(PHINT*0.5)+ 5.0           
!              P3      = ((TLNO + 0.5) * PHINT) - SUMDTT 
! JIL 09/10/2007 This was changed to make it compatible with new Leaf Area model
!        Assumes: 2nd leaf emerges 20 GDD after emergence (Jones & Kiniry, 1986)
!                 expansion of flag leaf takes 2 PHINT (Same, and Grant, 1989)
              P3      = (TLNO * PHINT) + 20.0 - SUMDTT 
              XNTI    = XN
...
! JIL 09/11/2007 Thermal time after emergence
	        GDDAE = GDDAE + DTT
              ! NDAS - number of days after sowing
              NDAS   = NDAS + 1            
...
C ** JIL Continuous phenological scale (0=Emergence; 1=Flowering; 2= Physiol Mat)
		      Z2STAGE = 0.35 + 0.65 * SUMDTT/P3
		      Z2STAGE = AMIN1(1.0, Z2STAGE)
...
!     CHP 5/25/2007 Move inflection point back to end of stage 3
              SeedFrac = SUMDTT / P5
C ** JIL Continuous phenological scale (0=Emergence; 1=Flowering; 2= Physiol Mat)
	        Z2STAGE = 1.0 + SUMDTT/P5
	        Z2STAGE = AMIN1(2.0, Z2STAGE)
              ...
              !   New Growth Stage Occurred Today. Initialize Some Varia
              !---------------------------------------------------------
! JIL 09/11/2007 Kernel number calculation was moved to a new routine (KNUMBER)
...
!             SeedFrac = (SUMDTT - DSGFT) / (P5 - DSGFT)
              SeedFrac = SUMDTT / P5
! JIL 09/11/2007 Continuous phenological scale (0=Emergence; 1=Flowering; 2= Physiol Mat)
		      Z2STAGE = 1.0 + SUMDTT/P5
		      Z2STAGE = AMIN1(2.0, Z2STAGE)
